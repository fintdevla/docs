...
<Tabs>
  <Tab title="Pago manual">
    ...
    Si el pago se cargó por error, usá **Eliminar pago manual**. Eso es la forma de **cancelar o deshacer un cobro cargado a mano**: revierte el importe y vuelve a dejar pendiente lo imputado a las órdenes.

    <Warning>
      Solo se pueden eliminar pagos **manuales**. Fint bloquea la acción si el pago no es manual o si ya tiene **comprobantes emitidos**. El botón se llama **Eliminar pago manual**, no Cancelar.
    </Warning>
  </Tab>
...
## Cancelar o eliminar un pago

No hay un botón que diga **Cancelar pago**.

| Qué querés hacer | Qué usar |
| --- | --- |
| Deshacer un cobro que cargaste a mano | En el detalle, **Eliminar pago manual** |
| Devolver un cobro online | Hacer el reembolso en el **procesador** (Mercado Pago, Pagos360, Nuvei u otro). Fint lo refleja como Reembolsado |
| El pagador desconoció el cobro | El procesador informa el contracargo. Fint lo refleja como Contracargo |

<Steps>
  <Step title="Abrí el pago">
    Entrá a **Pagos**, buscá el cobro y abrí el detalle.
  </Step>
  <Step title="Comprobá que sea manual">
    Junto al medio debe verse **(M)** o la marca Manual. Si es online, no aparece **Eliminar pago manual**.
  </Step>
  <Step title="Eliminar pago manual">
    Confirmá. El dinero deja de estar imputado y la orden vuelve a Por Cobrar o a Pago parcial, según lo que quede.
  </Step>
</Steps>

Si Fint dice que no se puede eliminar, suele ser porque **ya hay un comprobante de ARCA** sobre ese pago. Primero hay que resolver el comprobante; no fuerces la baja.
...
<CardGroup cols={2}>
  ...
  <Card title="Gestionar Órdenes" icon="file-invoice" href="/docs/guides/ordenes-gestionar">
